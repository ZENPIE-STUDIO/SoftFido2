...
static boolean_t isLogFileInitOK = false;

io_service_t getService(boolean_t bDriverKit) {
    io_service_t service;
    if (bDriverKit) {
        // DriverKit
        //softu2f_log(ctx, "DriverKit\n");
        service = IOServiceGetMatchingService(kIOMasterPortDefault, IOServiceNameMatching(kSoftFidoDriverClassName));
    } else {
        // KEXT
        //softu2f_log(ctx, "KEXT\n");
...
void initLogFile() {
    ...
    strcat(fullPath, kSoftU2FLogOutputFolder);
    strcat(fullPath, "/");
    strcat(fullPath, fileName);
    
    FILE *logFile = fopen(fullPath,"a");    // append
    log_add_fp(logFile, LOG_DEBUG);
    softu2f_log(ctx, "Init output log.\n");
}
#endif
// Initialize libSoftU2F before usage.
softu2f_ctx *softu2f_init(softu2f_init_flags flags, boolean_t bDriverKit) {
    // ----
    FILE *logFile = fopen("/Library/Application Support/MobileIron Authenticator/DaemonLog/softu2f.txt","w");
    ...
    kern_return_t ret;
    int err;
    const bool debug = (flags & SOFTU2F_DEBUG) == 1;
    // For 輸出檔案
#ifdef OUTPUT_SOFTU2F_LOGFILE
    if (debug && isLogFileInitOK == false) {
        initLogFile();
        isLogFileInitOK = true;
    }
#endif
    // Allocate a new context.
    ctx = (softu2f_ctx *)calloc(1, sizeof(softu2f_ctx));
    if (!ctx) {
        softu2f_log(NULL, "calloc softu2f_ctx failed!");
        return NULL;
    }
    ...
    if (!service) {
        softu2f_log(ctx, "Cannot find driver.\n");
        goto fail;
    }
    // Open connection to user client.
    ret = IOServiceOpen(service, mach_task_self(), 0, &ctx->con);
    if (ret != kIOReturnSuccess) {
        softu2f_log(ctx, "Cannot connecting to driver (IOServiceOpen): %d\n", ret);
        softu2f_log(ctx, "       system err = %X\n", err_get_system(ret)); // sys_iokit (38)
        softu2f_log(ctx, "          sub err = %X\n", err_get_sub(ret));
        softu2f_log(ctx, "         code err = %X\n", err_get_code(ret));
        goto fail;
    }
...
void softu2f_deinit(softu2f_ctx *ctx) {
    kern_return_t ret;
    // (For DriverKit)
//    if (ctx->outputBufferArray) {
//        free(ctx->outputBufferArray);
//        ctx->outputBufferArray = NULL;
//    }
...
int softu2f_run(softu2f_ctx *ctx) {
    ...
    if (isDriverKit) {
        async_ref[kIOAsyncCalloutFuncIndex] = (uint64_t) softu2f_driverkit_async_callback;
    } else {
        // KEXT
        async_ref[kIOAsyncCalloutFuncIndex] = (uint64_t) softu2f_kext_async_callback;
    }
    ...
    while (nextMsg) {
        ...
        if (softu2f_hid_msg_is_complete(ctx, msg)) {
            if (msg->buf != NULL) {
                ...
                softu2f_log(ctx, "[??] msg->buf is NULL\n");
            }
...
void softu2f_hid_msg_handler_register(softu2f_ctx *ctx, uint8_t type, softu2f_hid_message_handler handler) {
    switch (type) {
        case U2FHID_PING: ctx->ping_handler = handler; break;
        case U2FHID_MSG:  ctx->msg_handler = handler;  break;
        case U2FHID_INIT: ctx->init_handler = handler; break;
        case U2FHID_WINK: ctx->wink_handler = handler; break;
        case U2FHID_SYNC: ctx->sync_handler = handler; break;
        case U2FHID_CBOR: ctx->cbor_handler = handler; break;
    }
}
...
bool softu2f_hid_msg_handle_init(softu2f_ctx *ctx, softu2f_hid_message *req) {
    ...
    resp_data.versionInterface = U2FHID_IF_VERSION; // 2
    resp_data.versionMajor = 0;
    resp_data.versionMinor = 0;
    resp_data.versionBuild = 0;
//  resp_data.capFlags |= CAPFLAG_WINK; // U2FHID
    resp_data.capFlags |= (CAPFLAG_WINK | CAPFLAG_CBOR | CAPFLAG_NMSG);    // FIDO2 - MI_WINDOWS
//    resp_data.capFlags |= (CAPFLAG_CBOR | CAPFLAG_NMSG);
//  resp_data.capFlags |= (CAPFLAG_WINK | CAPFLAG_CBOR);    // FIDO2
...
softu2f_hid_message *softu2f_hid_msg_alloc(softu2f_ctx *ctx) {
    softu2f_hid_message *msg = (softu2f_hid_message *)calloc(1, sizeof(softu2f_hid_message));
...
void softu2f_inner_async_callback(softu2f_ctx *ctx, U2FHID_FRAME* frame) {
    if (frame == NULL) {
        softu2f_log(ctx, "[async_callback] frame is NULL!!\n");
        return;
    }
...
void softu2f_driverkit_async_callback(void *refcon, IOReturn result, uint64_t* args, uint32_t numArgs) {
    ...
    if (!refcon || result != kIOReturnSuccess) {
        softu2f_log(ctx, "Unexpected call to softu2f_driverkit_async_callback.\n");
        goto stop;
    }
    ctx = (softu2f_ctx *)refcon;
    
    //printf("[FIDO] Index = %u,  Max Count = %zu\n", numArgs, ctx->outputBufferArrayCount);
    softu2f_log(ctx, "[FIDO] Index = %u,  Max Count = %zu\n", numArgs, ctx->outputBufferArrayCount);
    // 利用App端準備好的OutputBuffer，就可以成功拿到裡面的資料
    if (numArgs < ctx->outputBufferArrayCount) {
        U2FHID_FRAME* frame = &(ctx->outputBufferArray[numArgs]);
        softu2f_inner_async_callback(ctx, frame);
    }
    // Eddie: 有無 Copy一份，都沒有影響
    //U2FHID_FRAME frame = {0};
    //memcpy(&frame, args, sizeof(U2FHID_FRAME));
    U2FHID_FRAME* frame = (U2FHID_FRAME*) args;
    softu2f_inner_async_callback(ctx, frame);
//    softu2f_log(ctx, "[FIDO] Index = %u,  Max Count = %zu\n", numArgs, ctx->outputBufferArrayCount);
    // 利用App端準備好的OutputBuffer，可以成功拿到裡面的資料
//    if (numArgs < ctx->outputBufferArrayCount) {
//        U2FHID_FRAME* frame = &(ctx->outputBufferArray[numArgs]);
//        softu2f_inner_async_callback(ctx, frame);
//    }
...
void softu2f_kext_async_callback(void *refcon, IOReturn result, io_user_reference_t* args, uint32_t numArgs) {
    softu2f_ctx *ctx = NULL;
    U2FHID_FRAME *frame = NULL;

    if (!refcon || result != kIOReturnSuccess) {
        softu2f_log(ctx, "Unexpected call to softu2f_kext_async_callback.\n");
        goto stop;
    }

    ctx = (softu2f_ctx *)refcon;
    if (numArgs * sizeof(io_user_reference_t) != sizeof(U2FHID_FRAME)) {
        softu2f_log(ctx, "Unexpected argument count in softu2f_kext_async_callback.\n");
        goto stop;
    }
...
void softu2f_async_timer_callback(CFRunLoopTimerRef timer, void* info) {
    ...
    if (!ctx) {
        softu2f_log(ctx, "[async_timer_callback] ctx is NULL\n");
        goto stop;
    }
    const int kOutputCount = 300;    // 300(60秒)
    static int staticCount = 0;
    staticCount++;
    io_service_t service = getService(isDriverKit);
    if (!service) {
        softu2f_log(ctx, "async_timer_callback GetMatchingService ❌\n");
        goto stop;
    }
    
    if (staticCount > kOutputCount) {
        staticCount = 0;
        softu2f_log(ctx, "async_timer_callback GetMatchingService ✅\n");
    }
